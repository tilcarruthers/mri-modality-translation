...
def test_dataset_wrapper_returns_expected_keys_and_shape():
    ...
    normalizer = RangeNormalizer(stats={"t1": {"min": 0.0, "max": 3.0}, "t2": {"min": 1.0, "max": 4.0}})
    ds = MRIPairedHFDataset(dummy, input_key="t1", target_key="t2", normalizer=normalizer, metadata_keys=["view"])
    sample = ds[0]
