...
REQUIRED_TOP_LEVEL_KEYS = {"experiment", "seed", "data", "loader", "model", "training", "evaluation"}
...
def validate_train_config(config: dict[str, Any]) -> None:
    ...
    _require_keys(config["data"], {"dataset_name", "train_split", "val_split", "input_key", "target_key"}, "data")
    _require_keys(config["loader"], {"batch_size", "num_workers", "pin_memory"}, "loader")
    _require_keys(config["model"], {"name"}, "model")
    _require_keys(config["training"], {"epochs", "lr", "loss", "device", "use_amp", "compile_model"}, "training")
